...
public final class ExtensionControl {
	...
	public static void func(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (arg1.equals("list")) {
			...
			for (String ext : extensions.getActiveExtensions().stream().map(e -> e.getName())
					.sorted((s1, s2) -> s1.compareToIgnoreCase(s2)).collect(Collectors.toList()))
				sender.sendMessage(Text.colorize("  &b", ext));
			for (String ext : extensions.getInactiveExtensions().stream().map(e -> e.getName())
					.sorted((s1, s2) -> s1.compareToIgnoreCase(s2)).collect(Collectors.toList()))
			 ...
			 * 
			 * if(args.length > 1) { String arg2 = args[1].toLowerCase(Locale.US);
			 * 
			 * List<Path> extensionFiles; try { extensionFiles =
			 * Files.list(QuestWorldPlugin.getPath("data.extensions").toPath()) .filter(path
			 * -> !Files.isDirectory(path)) .collect(Collectors.toList()); } catch
			 * (Exception e) {
			 * sender.sendMessage(Text.colorize("&cError reading extensions, see console"));
			 * Log.warning("Could not read extensions"); e.printStackTrace(); return; }
			 * 
			 * for(Path extensionFile : extensionFiles) { String fileName =
			 * extensionFile.toFile().getName(); if(fileName.equalsIgnoreCase(arg2)) {
			 * if(useEnable) { if(fileName.endsWith(".jar")) {
			 * sender.sendMessage(Text.colorize("&cExtension ", arg2,
			 * " is already enabled")); } else { enable(sender, extensionFile); } } else {
			 * if(!fileName.endsWith(".jar")) {
			 * sender.sendMessage(Text.colorize("&cExtension ", arg2, " is not enabled")); }
			 * else { disable(sender, extensionFile); } }
			 * 
			 * return; } } sender.sendMessage(Text.colorize("&cExtension ", arg2,
			 * " was not found"));
			 * 
			 * } else sender.sendMessage(Text.colorize("&c/", label, " extension ", type,
			 * " <extension>"));
			 */
		}
	}
	...
	/*
	 * private static void enable(CommandSender sender, Path file) { String fileName
	 * = file.toFile().getName(); fileName = fileName.substring(0,
	 * fileName.lastIndexOf('.')) +".jar"; try { file = Files.move(file,
	 * file.resolveSibling(fileName)); } catch (Exception e) {
	 * sender.sendMessage(Text.colorize("&cError enabling extension, see console"));
	 * Log.warning("Error enabling extension " + fileName); e.printStackTrace();
	 * return; }
	 * 
	 * QuestWorldPlugin.getImpl().getPlugin().getLoader().load(file.toFile()); }
	 * 
	 * private static void disable(CommandSender sender, Path file) { String
	 * fileName = file.toFile().getName(); fileName = fileName.substring(0,
	 * fileName.lastIndexOf('.')) +".disabled"; try { Files.move(file,
	 * file.resolveSibling(fileName)); } catch (IOException e) {
	 * sender.sendMessage(Text.colorize("&cError enabling extension, see console"));
