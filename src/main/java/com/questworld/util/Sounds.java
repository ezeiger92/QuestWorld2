...
public class Sounds {
	...
	public Sounds(ConfigurationSection config) {
		...
		DESTRUCTIVE_CLICK = new SoundList(config.getConfigurationSection("sounds.editor.destructive-action-click"),
				0.5f, 0.5f);
		...
		public SoundList(ConfigurationSection config, float volume, float pitch) {
			...
			if (soundStr == null) {
				...
				if (!sounds.isEmpty()) {
					...
					Log.warning("Sound path \"" + config.getCurrentPath()
							+ "\" uses an old format, using first sound in list (" + soundStr + ")");
