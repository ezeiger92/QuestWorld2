...
public class QBDialogue {
	public static void openDeletionConfirmation(Player p, final DataObject q) {
		...
		menu.put(6, ItemBuilder.Proto.RED_WOOL.get().display("&cNo").get(), event -> {
			...
			else if (q instanceof ICategory)
				QuestBook.openCategoryList(p2);
			else if (q instanceof IMission)
				QuestBook.openQuestEditor(p2, ((IMission) q).getQuest());
		});

		String tag;
		if (q instanceof IQuest)
			tag = "your Quest \"" + ((IQuest) q).getName() + "\"";
		else if (q instanceof ICategory)
			tag = "your Category \"" + ((ICategory) q).getName() + "\"";
		else if (q instanceof IMission)
			tag = "your Task";
		else
			tag = "";

		menu.put(2, ItemBuilder.Proto.LIME_WOOL.get().wrapText("&aYes I am sure", "", "&rThis will delete", tag).get(),
				event -> {
					Player p2 = (Player) event.getWhoClicked();
	...
	public static void openQuestMissionEntityEditor(Player p, final IMission mission) {
		...
		for (int i = 0; i < entities.length; ++i) {
			...
			pager.addButton(i, EntityTools.getEntityDisplay(entity)
					.wrapText("&7Entity Type: &e" + EntityTools.nameOf(entity), "", "&e> Click to select").get(),
					event -> {
	...
	public static void openCommandEditor(Player p, IQuest quest) {
		...
		for (int i = 0; i < quest.getCommands().size(); i++) {
			...
			int index = i;
			Prop remove = FUSE(HOVER_TEXT("Click to remove this Command", GRAY), CLICK_RUN(p, () -> {
			...
			Prop above = FUSE(HOVER_TEXT("Click to insert above", GRAY), CLICK_RUN(p, () -> {
				PlayerTools.promptCommand(p, new SinglePrompt("&7Type in your desired Command:", (c, s) -> {
					IQuestState changes = quest.getState();
		...
		Prop add = FUSE(HOVER_TEXT("Click to add a new Command", GRAY), CLICK_RUN(p, () -> {
			PlayerTools.promptCommand(p, new SinglePrompt("&7Type in your desired Command:", (c, s) -> {
				IQuestState changes = quest.getState();
		...
		PlayerTools.tellraw(p,
				new JsonBlob("+ ", DARK_GREEN, add).add("Add more Commands... (Click)", GRAY, add).toString());

		Prop back = FUSE(HOVER_TEXT("Open quest editor", GRAY),
				CLICK_RUN(p, () -> QuestBook.openQuestEditor(p, quest)));

		PlayerTools.tellraw(p, new JsonBlob("< ", BLUE, back).add("Return to quest editor", GRAY, back).toString());
		...
		for (IQuest quest : category.getQuests()) {
			if (!cycleDetection(q, quest))
				pager.addButton(quest.getID(), new ItemBuilder(quest.getItem()).wrapText(quest.getName(), "",
						"&e> Click to set requirement for " + (isQuest ? "quest" : "category") + ": &f&o" + name).get(),
						event -> {
							Player p2 = (Player) event.getWhoClicked();
